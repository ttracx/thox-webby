...
| 8.6 | Validation pending | Restore scheduled janitor | Reusable workflow invocation is accepted and creates a job | Caller forwards only secrets declared by the pinned reusable workflow; dispatch and inspect the replacement run | Confirm the new scheduled/manual run |
