...
| 8.6 | Validation pending | Restore scheduled maintenance | A scheduled/manual smoke workflow creates a job and passes | Read-only permissions, no secrets, `npm ci` plus `npm test` | Confirm the replacement run |
